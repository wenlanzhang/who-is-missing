...
#
# Then:  python analysis/01_build_panel.py   (and 02-08)
...
print('\n'.join(c for c in rc.list_cities() if c.startswith('$2_')))")
      REGIONS+=("${found[@]}")
      shift 2 ;;
    --all)
      mapfile -t found < <("$PYTHON" -c "
...
echo ""
echo "Done. Next:"
echo "  python analysis/01_build_panel.py"
echo "  python analysis/02_selection_models.py   # ... through 08"
